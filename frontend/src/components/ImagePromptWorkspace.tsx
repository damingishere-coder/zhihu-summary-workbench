...
export function ImagePromptWorkspace({ draftId }: { draftId: string }) {
  ...
  const [template, setTemplate] = useState<"knowledge_card" | "comparison_table">("knowledge_card");
  const [canvasSize, setCanvasSize] = useState<"1080x1440" | "1242x1660">("1080x1440");
  const [fontScale, setFontScale] = useState(1);
  const [brand, setBrand] = useState("知乎问题总结工作台");
  const [footer, setFooter] = useState("内容由 AI 辅助整理，请结合来源人工核验");
  ...
  const refresh = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: ["publish-readiness", draftId] });
  };
  ...
  const generateContent = useMutation({
    mutationFn: () => api.generateInfographicContent(draftId, {
  ...
  const generatePrompt = useMutation({
    mutationFn: () => api.generateImagePrompt(draftId, { visual_style: style, aspect_ratio: "3:4" }),
    onSuccess: async () => {
  ...
  const render = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.current?.overflow.length) message.warning("检测到文字溢出，请精简内容");
  ...
  const restore = useMutation({
    mutationFn: (version: number) => api.restoreImageVersion(workspace!.id, version),
    onSuccess: async () => {
  ...
  const cssMode = useMutation({
    mutationFn: (enabled: boolean) => api.setImageCssMode(workspace!.id, enabled),
    onSuccess: refresh,
    onError: (error) => message.error(error.message),
  });
  ...
  const copy = async (language: "zh" | "en") => {
    if (!current) return;
    try {
      await copyPromptText(language === "zh" ? current.prompt_zh : current.prompt_en);
  ...
  const contentCount = useMemo(
    () => content.consensus.length + content.disagreements.length + content.conditions.length + content.suggestions.length,
    [content],
  );
  ...
  return (
    <section className="image-prompt-workspace">
      <div className="section-heading">
        <div>
          <h2>信息图与手动图片 Prompt</h2>
          <p>文案和中文由 HTML/CSS 准确渲染；背景图可在 ChatGPT 手动生成。</p>
        </div>
      ...
      <Alert
        type="info"
        showIcon
        title="手动图片 Prompt 工作流位于草稿审核"
        description="不调用 OpenAI 图片 API；复制 Prompt、手动生图、上传背景后再由本机 Playwright 合成文字。"
      />
      {!current ? (
        <div className="image-workspace-empty">
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="先根据观点地图生成有长度约束的信息图文案" />
          <Space wrap>
            <Select
              value={template}
        ...
        <>
          ...
          <div className="image-workspace-summary">
            <span>{template === "knowledge_card" ? "知识总结卡" : "观点对比表"}</span>
            <span>{contentCount} 个内容点</span>
            <Tag color={current.render_status === "rendered" ? "success" : "default"}>
      ...
      <Drawer
        ...
        extra={
          <Space>
            <Button icon={<SaveOutlined />} loading={save.isPending} onClick={() => save.mutate()}>
              保存新版本
            </Button>
            <Button type="primary" icon={<PictureOutlined />} loading={render.isPending} onClick={() => render.mutate()}>
              渲染 PNG
            </Button>
          </Space>
        }
          ...
          <div className="infographic-editor-layout">
                    ...
                    children: (
                      <div className="infographic-control-stack">
                        ...
                        <Input.TextArea
                          ...
                          onChange={(event) => setContent({ ...content, one_line_conclusion: event.target.value })}
                        />
                        <div className="info-list-editor">
                          <div className="info-list-editor__heading"><strong>共识</strong><span>{content.consensus.length}/4</span></div>
                          {content.consensus.map((item, index) => (
                            <div
                              className="consensus-editor-row"
                              key={`${item.title}-${index}`}
                              draggable
                              onDragStart={(event) => event.dataTransfer.setData("consensus/index", String(index))}
                              onDragOver={(event) => event.preventDefault()}
                              onDrop={(event) => {
                                const from = Number(event.dataTransfer.getData("consensus/index"));
                            ...
                            >
                              ...
                              <div className="consensus-editor-fields">
                                <Input
                                  ...
                                  onChange={(event) => setContent({
                                    ...content,
                                    consensus: content.consensus.map((value, itemIndex) =>
                              ...
                              <Button type="text" danger icon={<DeleteOutlined />} onClick={() => setContent({
                                ...content,
                                consensus: content.consensus.filter((_, itemIndex) => itemIndex !== index),
                              })} />
                            </div>
                          ))}
                        </div>
                        <StringListEditor title="主要分歧" values={content.disagreements} onChange={(values) => setContent({ ...content, disagreements: values })} />
                        <StringListEditor title="适用条件" values={content.conditions} onChange={(values) => setContent({ ...content, conditions: values })} />
                        <StringListEditor title="行动建议" values={content.suggestions} onChange={(values) => setContent({ ...content, suggestions: values })} />
                      </div>
                    ),
                  },
                  {
                    ...
                    children: (
                      <div className="infographic-control-stack">
                        ...
                        <label>画布尺寸</label>
                        <Select value={canvasSize} onChange={setCanvasSize} options={[
                          { value: "1080x1440", label: "1080 × 1440（推荐）" },
                          { value: "1242x1660", label: "1242 × 1660" },
                        ]} />
                        <label>字号缩放 {fontScale.toFixed(2)}</label>
                        <Slider min={0.8} max={1.25} step={0.05} value={fontScale} onChange={setFontScale} />
                        <label>品牌名</label>
                        <Input value={brand} maxLength={32} onChange={(event) => setBrand(event.target.value)} />
                        <label>页脚说明</label>
                        <Input value={footer} maxLength={64} onChange={(event) => setFooter(event.target.value)} />
                        <div className="background-workflow__heading">
                          <div><strong>背景来源</strong><p>未上传时保持纯 CSS，上传失败不阻塞。</p></div>
                          <Space><span>纯 CSS</span><Switch checked={workspace.use_css_background} loading={cssMode.isPending} onChange={(enabled) => cssMode.mutate(enabled)} /></Space>
                        </div>
                        <Upload
                          accept="image/png,image/jpeg,image/webp"
                          showUploadList={false}
                          customRequest={({ file, onSuccess, onError }) => upload.mutate(file as File, {
                            onSuccess: () => onSuccess?.({}),
                            onError: (error) => onError?.(error),
                          })}
                        >
                          <Button icon={<UploadOutlined />} loading={upload.isPending}>上传或替换手动背景图</Button>
                        </Upload>
                        {current.background_url && (
                          <Popconfirm title="移除当前背景图？" description="历史版本与原始文件仍保留。" onConfirm={() => remove.mutate()}>
                            <Button danger icon={<DeleteOutlined />}>移除当前背景</Button>
                          </Popconfirm>
                        )}
                        <label>背景水平位置 {backgroundX}%</label>
                        <Slider min={0} max={100} value={backgroundX} onChange={setBackgroundX} />
                        <label>背景垂直位置 {backgroundY}%</label>
                        <Slider min={0} max={100} value={backgroundY} onChange={setBackgroundY} />
                        <label>背景缩放</label>
                        <InputNumber min={1} max={2} step={0.1} value={backgroundScale} onChange={(value) => setBackgroundScale(value ?? 1)} />
                      </div>
                    ),
                  },
                  {
                    ...
                    children: (
                      <div className="prompt-copy-panel">
                        <Alert type="info" showIcon title="这里只生成 Prompt，不调用图片 API" description="复制到 ChatGPT 手动生图后，在“版式与背景”上传原图。" />
                        ...
                        <Button icon={<ReloadOutlined />} loading={generatePrompt.isPending} onClick={() => generatePrompt.mutate()}>生成新 Prompt 版本</Button>
                        <label>中文 Prompt</label>
                        <Input.TextArea readOnly autoSize={{ minRows: 8, maxRows: 14 }} value={current.prompt_zh} />
                        <Button icon={<CopyOutlined />} disabled={!current.prompt_zh} onClick={() => void copy("zh")}>复制中文 Prompt</Button>
                        <label>英文 Prompt</label>
                        <Input.TextArea readOnly autoSize={{ minRows: 8, maxRows: 14 }} value={current.prompt_en} />
                        <Button icon={<CopyOutlined />} disabled={!current.prompt_en} onClick={() => void copy("en")}>复制英文 Prompt</Button>
                      </div>
                    ),
                  },
                  {
                    key: "history",
                    label: <><HistoryOutlined /> 历史 {workspace.versions.length}</>,
                    children: (
                      <div className="image-version-list image-version-list--actions">
                        {workspace.versions.map((version) => (
                          <div key={version.id}>
                            ...
                            <Tag color={version.render_status === "rendered" ? "success" : "default"}>{version.render_status}</Tag>
                            <Button size="small" loading={restore.isPending} onClick={() => restore.mutate(version.version)}>恢复为新版本</Button>
                          </div>
                        ))}
                      </div>
                    ),
            ...
            <div className="infographic-editor-preview">
              <div className="infographic-editor-preview__heading">
                <div><strong>实时预览</strong><span>{canvasSize.replace("x", " × ")}</span></div>
                {current.overflow.length > 0 && <Tag color="error">文字溢出 {current.overflow.length} 处</Tag>}
              </div>
              <InfographicPreview
                content={content}
                template={template}
              ...
              {current.render_log.length > 0 && (
                <Alert
                  type={current.render_status === "rendered" ? "success" : "warning"}
                  showIcon
              ...
              {current.download_url && (
                <Button block type="primary" icon={<DownloadOutlined />} href={current.download_url}>下载当前 PNG</Button>
              )}
            </div>
          </div>
        )}
