...
- 扩展 `daily_plans`，保存定时执行、并发、回答上限、发布上限、发布间隔和自动开关；
- 扩展 `model_usage_logs`，保存阶段、缓存命中、降级和重试次数；
- 新增 `model_response_cache`，对结构化文本模型结果做内容哈希缓存；
- 扩展 `prompt_versions`，保存参数、测试输入输出、变更说明和创建人；
- 扩展 `image_versions`，保存缩略图、HTML 快照、画布、渲染状态、溢出检查和日志；
- 扩展 `publish_schedules`，固定文章与图片版本并记录顺序、间隔、确认和执行时间；
- 扩展 `publish_records`，保存最终文章/图片版本、截图和尝试次数。

第三阶段迁移继续只增加结构，不删除旧列、不改写历史版本。渲染文件和上传文件保存在 Git 忽略的 `data/`，数据库只保存受控路径引用和元数据。
