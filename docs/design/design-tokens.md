...
- 默认画布：`1080×1440`，备选 `1242×1660`。
- 安全区：左右 72px、顶部 72px、底部 64px。
- 信息图标题：56–68px，最多 2 行。
- 一句话结论：34–42px，最多 3 行。
- 区块标题：28–34px；区块正文：24–30px。
- 预览按画布等比缩放，不用缩小真实字号来掩盖溢出。
- 知识总结卡使用单列或双列分组；观点对比表使用明确的两侧栏和中性分隔线。
- 上传背景始终加可调遮罩，保证正文对比度；纯 CSS 模式不使用渐变或装饰性 CSS 绘画。
- 溢出使用错误色边框、字段帮助文字和预览定位共同表达。

## 第三阶段日历与成本

- 日历事件最小高度 28px，状态文字不能只靠颜色。
- 冲突使用警告底色和图标，不覆盖日期文字。
- 成本数字使用等宽数字特性，费用保留 4 位小数。
- 预算进度超过 80% 使用警告色，超过 100% 使用错误色并显示暂停原因。
